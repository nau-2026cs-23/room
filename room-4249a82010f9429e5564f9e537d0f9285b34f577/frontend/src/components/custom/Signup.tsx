  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success && data.data?.token) {
        ...
        toast.success('注册成功', { description: `欢迎加入学资库，${name}！已赠送100积分` });
        navigate('/', { replace: true });
      } else {
    ...
    <div className="min-h-screen bg-[#F4F6FA] flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-[#0F1C35] mb-1">创建账号</h1>
          <p className="text-[#5A6A85] text-sm">加入学资库，获取海量优质学习资料</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-[0_10px_24px_-4px_rgb(26_58_107_/_0.15)] p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="name" className="text-[#0F1C35] font-medium">姓名</Label>
              <Input
                id="name"
                type="text"
                placeholder="请输入你的姓名"
                ...
                className="h-11 border-[#D8E0EE] focus:border-[#2E6BE6]"
              />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="confirmPassword" className="text-[#0F1C35] font-medium">确认密码</Label>
              <Input
                id="confirmPassword"
                type="password"
                placeholder="再次输入密码"
            ...
            <Button
              type="submit"
              disabled={loading}
              className="w-full h-11 bg-[#1A3A6B] hover:bg-[#2E6BE6] text-white font-semibold rounded-xl transition-all duration-200 shadow-[0_4px_12px_-2px_rgb(26_58_107_/_0.3)] mt-2"
            >
              {loading ? '注册中...' : '免费注册'}
            </Button>
          </form>
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-[#5A6A85]">
              已有账号？{' '}
              <Link to="/login" className="text-[#2E6BE6] font-medium hover:underline">
                立即登录
              </Link>
            </p>
          </div>
